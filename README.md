...
* แสตมป์ Passport ออกให้เฉพาะคนที่ **เช็คอินหน้างาน** เท่านั้น (ยอดร่วมบุญที่อนุมัติแล้วไม่ให้แสตมป์เอง แต่เพิ่มดวงฟอยล์ `tier` ให้เมื่อเช็คอินงานนั้นแล้วและยอดถึง ฿500) — `server/passport.js`
...
* **รูปหมู่**: รูปที่นับหน้าได้ตั้งแต่ `GROUP_MIN_FACES` (ค่าเริ่มต้น 20) คนขึ้นไป ถูกทำเครื่องหมาย `group_ok` ให้อัตโนมัติตอนสแกน · แอดมินเพิ่ม/เอาออกเองได้ต่อรูปหรือหลายรูป และกดทำเครื่องหมายอัตโนมัติซ้ำได้ · ตัวเลือก «รูปหมู่» ของสมาชิกเห็นเฉพาะรูปที่ทำเครื่องหมายไว้ (ไม่มีเลย → ใช้เกณฑ์ ≥ 20 คนตรง ๆ)
...
* ล็อกอินซ้ำ (LINE/Google) จะ **ไม่ทับชื่อและรูปที่สมาชิกตั้งเอง** — ชื่อดึงจากผู้ให้บริการเฉพาะตอนสมัครครั้งแรก · รูปอัปเดตจากผู้ให้บริการเฉพาะเมื่อ `users.avatar_custom = 0` (อัปโหลดรูปเองเมื่อไหร่ ธงนี้ตั้งเป็น 1 ถาวร)
