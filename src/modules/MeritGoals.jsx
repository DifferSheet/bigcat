...
export default function MeritGoals({ data }) {
  ...
  return <>
    ...
    {cfg.attend?.enabled && <Section eyebrow="JOIN IN PERSON" title="ไปวัดด้วยกัน" aside={<span className="ev-summary">จะไป <strong>{registrations?.total || 0}</strong> คน · เช็คอินแล้ว {registrations?.checkedIn || 0}</span>}>
      ...
      {attendDone
        ? <><Notice>ลงทะเบียนไปวัดแล้ว หมายเลข #{String(attendDone.number).padStart(3, '0')} — วันงาน{(cfg.checkinMode || 'self') === 'staff' ? 'เปิดหน้าบัตรให้พี่ ๆ สแกน QR' : cfg.checkinMode === 'gate' ? 'สแกน QR ที่จุดเช็คอิน' : 'เปิดหน้าบัตรกดเช็คอิน (หรือให้พี่ ๆ สแกน QR)'} เพื่อรับของที่ระลึก</Notice><div className="form-actions"><Link className="button ghost small" to={`/ticket/${attendDone.code}`}>เปิดบัตร</Link><button className="link-button" onClick={() => { setAttendDone(null); try { localStorage.removeItem(attendKey(ev.slug)); } catch { /* optional */ } }}>ลงทะเบียนคนอื่น</button></div></>
