...
const TABS = [['profile', 'โปรไฟล์', 'user'], ['photos', 'รูปของฉัน', 'camera'], ['history', 'รายการของฉัน', 'clock']];
...
export default function AccountPage() {
  ...
  // แท็บอยู่ใน URL (?tab=photos) — แชร์ลิงก์/กด back ได้ · #face ของลิงก์เดิมพามาที่แท็บรูปของฉัน
  const [tab, setTab] = useState('profile');
  const navigate = useNavigate();
  useEffect(() => {
    const q = new URLSearchParams(location.search).get('tab');
    setTab(TABS.some(t => t[0] === q) ? q : location.hash === '#face' ? 'photos' : 'profile');
  }, []);
  const go = (next) => { setTab(next); navigate(next === 'profile' ? '/account' : `/account?tab=${next}`, { replace: true }); };
  ...
  return <>
    ...
    <main className="ev-page">
      ...
      <nav className="account-tabs" aria-label="ส่วนต่าง ๆ ของบัญชี">
        {TABS.map(([key, label, icon]) => <button key={key} type="button" className={tab === key ? 'active' : ''} aria-current={tab === key ? 'page' : undefined} onClick={() => go(key)}><Icon name={icon} size={16} /> {label}</button>)}
      </nav>
      ...
      {tab === 'profile' && <div className="account-layout">
        <Section eyebrow="PROFILE" title="ข้อมูลสำหรับจอง/สั่งซื้อ">
          ...
          <form className="booking-form" onSubmit={save}>
            <div className="two"><label>ชื่อที่ใช้<input value={form.display_name} onChange={e => setForm({ ...form, display_name: e.target.value })} /></label><label>เบอร์โทร<PhoneInput value={form.phone} onChange={phone => setForm(f => ({ ...f, phone }))} /></label></div>
      ...
      {tab === 'history' && <div className="account-layout">
        <Section eyebrow="HISTORY" title="รายการของฉัน">
          {!act ? <PageLoader /> : empty ? <Notice tone="muted">ยังไม่มีรายการที่ทำตอนล็อกอินอยู่ — รายการก่อนหน้านี้ค้นด้วยรหัสได้ที่ <Link to="/ticket/lookup">บัตรของฉัน</Link></Notice> : <>
            {act.orders?.length > 0 && <Group title="คำสั่งซื้อ">{act.orders.map(o => <Row key={o.code} to={`/order/${o.code}`} image={o.image || '/images/bigcat-merch.jpg'} main={<>#{o.code} · {baht(o.total)}</>} sub={`${fmt(o.created_at)} · ${o.items} ชิ้น`} status={o.status} />)}</Group>}
            ...
            {act.registrations?.length > 0 && <Group title="ลงทะเบียนร่วมงาน">{act.registrations.map(r => <Row key={r.code} to={`/ticket/${r.code}`} image={r.cover} main={<>{r.title} · หมายเลข {r.number}</>} sub={`${fmt(r.starts_at)}${r.checked_in_at ? ' · เช็คอินแล้ว' : ''}`} />)}</Group>}
