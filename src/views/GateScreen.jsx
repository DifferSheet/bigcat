...
// QR เปลี่ยนทุก 45 วิ (โทเคนจาก server · server ยังรับย้อนหลัง 2 ช่อง = สแกนแล้วมีเวลากรอก ≥ 2 นาที) → รูปถ่าย QR ที่ส่งต่อไปใช้จากที่บ้านไม่ได้ · ต้องล็อกอินแอดมินก่อนเปิดจอ
import React, { useEffect, useRef, useState } from 'react';
import { drawQr } from '../lib/qr.js';
import { Link } from '../lib/nav.jsx';
import { Flower, Logo, PageLoader, Tag } from '../components/ui.jsx';
...
export default function GateScreen({ slug }) {
  ...
  useEffect(() => {
    ...
    drawQr(canvas.current, `${location.origin}/checkin/${slug}?t=${gate.token}`, { size: 520 });
  }, [gate?.token, slug]);
      ...
      {error ? <div className="gate-msg"><h1>{error}</h1>{error.includes('เข้าสู่ระบบ') && <Link className="button dark" to="/admin">ไปหน้าเข้าสู่ระบบ</Link>}</div>
        : !gate ? <PageLoader />
          : <>
            ...
            {state === 'open' && <p className="gate-hint">เปิดกล้องมือถือ สแกน แล้วกด <Tag>เช็คอิน</Tag> ได้เลย</p>}
            ...
            {gate.mode !== 'gate' && <p className="small-note warn">⚠ งานนี้ตั้งวิธีเช็คอินเป็น <Tag>{gate.mode === 'staff' ? 'ทีมงานสแกนเท่านั้น' : 'กดเองได้'}</Tag> — QR นี้จะยังไม่ทำงาน เปลี่ยนใน <Tag>แก้ไขงาน</Tag> → <Tag>เช็คอินหน้างาน</Tag> → <Tag>สแกน QR หน้างาน</Tag></p>}
          </>}
    </section>
  </main>;
}
